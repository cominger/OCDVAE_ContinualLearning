...
class ResNetGenerator(nn.Module):
    ...
    def __init__(self, num_features=64, bottom_width=4,
        ...
        self.block2 = Block(num_features, self.num_features * 4,
                            activation=activation, upsample=True, 
                            num_classes=num_classes) 
        self.block3 = Block(self.num_features * 4, self.num_features * 2,
                            activation=activation, upsample=True,
                            num_classes=num_classes)
        self.block4 = Block(self.num_features * 2, self.num_features *2,
                            activation=activation, upsample=False,
                            num_classes=num_classes)
        self.block5 = nn.Conv2d(self.num_features*2, self.num_features, 1, 1)
